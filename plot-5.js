(function() {/* constants */
	const canvas = document.getElementById("plot-5");
	...
	function draw() {
		...
		const f = (x) => {
			return Math.exp(-1 * x * x) + 2 * Math.exp(-1 * (x - 3) * (x - 3)) - 2 * Math.exp(-1 * (x - 4) * (x - 4));
		};
		drawFunction(f, "orange");
		drawFunction((x) => {return f(parameterA * x);}, "brown");
	...
	/* slider */
	const sliderCanvas = document.getElementById("slider-2");
	const sliderCtx = sliderCanvas.getContext("2d");
	const paragraph = document.getElementById("f2-dilatazione");
